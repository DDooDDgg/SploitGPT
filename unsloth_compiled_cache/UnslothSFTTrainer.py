...
class _UnslothSFTTrainer(Trainer):
    ...
    def __init__(
        ...
        eos_token_arg = args.eos_token
        if eos_token_arg in {"<EOS_TOKEN>", "<|EOS_TOKEN|>"}:
            eos_token_arg = None
            args.eos_token = None

        if eos_token_arg is not None:
            eos_token = eos_token_arg
            ...
            args.eos_token = eos_token
        else:
            args.eos_token = tokenizer.eos_token

        pad_token_arg = args.pad_token
        if pad_token_arg in {"<PAD_TOKEN>", "<|PAD_TOKEN|>"}:
            pad_token_arg = None
            args.pad_token = None

        if pad_token_arg is not None:
            pad_token_id = tokenizer.convert_tokens_to_ids(pad_token_arg)
            if pad_token_id is None:
                raise ValueError(
                    f"The specified `pad_token` ('{pad_token_arg}') is not found in the vocabulary of the given "
                    f"`processing_class` ({processing_class.__class__.__name__}). Ensure that the `pad_token` exists "
                    "in the vocabulary before using it as a PAD token."
                )
            tokenizer.pad_token = pad_token_arg
            tokenizer.pad_token_id = pad_token_id
            args.pad_token = pad_token_arg
